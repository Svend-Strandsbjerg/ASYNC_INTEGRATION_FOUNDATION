...
def build_timesheet_commit_queue(queue_id: str = "timesheet-q") -> Queue:
    ...
    queue.items.extend(
        [
            QueueItem(id="ts-1", queue_id=queue.id, payload={"id": "ts-1", "hours": 8}, state=QueueItemState.STAGED),
            QueueItem(id="ts-2", queue_id=queue.id, payload={"id": "ts-2", "hours": 7.5}, state=QueueItemState.STAGED),
        ]
    )
